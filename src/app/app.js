(function () {
    var config = {
        get: {
            url: "weixin_menu.json",
            method: "get",
        },
        save: {
            url: "app/weixin_menu.json",
            method: "post",
        },
        query: {
            url: "queryMenu",
    ...
    app.controller('menu', function ($scope, $http, $window) {
        ...
        $scope.weixin = {name:"微信号", appid: "AppId", appsecret: "AppSecret",qrcodeurl:""};
        ...
        $scope.saveConfig = function () {
            $scope.configModalError = false;
            if ( $scope.weixin.name === ""||$scope.weixin.appid === "" || $scope.weixin.appsecret === "") {
                $scope.configModalError = true;
            }
            else {
                var api = new $scope.wechatApi($scope.weixin.appid, $scope.weixin.appsecret);
               var token=api.getAccessToken( function(err,result){
                   if(err){
                       alert("AppId或AppSecret设置不正确,请核对后!");
                   }
                   else{
                       $scope.weixin.qrcodeurl='http://open.weixin.qq.com/qr/code?username='+  $scope.weixin.name;
                       var isSuccess = $scope.writeConfig();
                               if (isSuccess == true){
                                   $scope.$apply();
                ...
                //通过获取永久二维码方式验证配置是否有效方式去掉
                //api.createLimitQRCode(100, function(err,result){
                //    if(err){
                //        alert("AppId或AppSecret设置不正确,请核对后!");
                //    } else{
                //        $scope.weixin.qrcodeurl=api.showQRCodeURL(result.ticket);
                //        var isSuccess = $scope.writeConfig();
                //        if (isSuccess == true){
                //            $scope.$apply();
                //            $('#configModal').modal('hide');
...
//		$scope.fs.readFile(config.get.url, "utf8", function (error, data) {
//		if (error) {				
//			console.log(error);
//			alert('读取菜单配置文件失败');
//		} else {
...
//	});
        ...
        $scope.query = function () {
            ...
            api.getMenu(function (err, result) {
                if (err) {
                    if(err.code===48001){
                        alert("查询菜单失败,没有自定义菜单权限。");
                    }else{
                        alert("查询菜单失败,错误码："+err.code+"。");
                    }
                } else {
                    load(result);
                    $scope.$apply();//需要手动刷新
        ...
        $scope.publish = function () {
            ...
            button.button = menu;
            var menuStr=JSON.stringify(button);
            console.log(menuStr);
            var api = new $scope.wechatApi($scope.weixin.appid, $scope.weixin.appsecret);
            api.createMenu(menuStr, function (err, result) {
                if (err) {
                    console.log(err);
                    if(err.code===48001){
                        alert("创建菜单失败,没有自定义菜单权限。");
                    }else{
                        alert("创建菜单失败,错误码："+err.code+"。");
                    }
                } else {
                    $scope.fs.writeFileSync(config.save.url, menuStr, {encoding: 'utf8'});
                    alert("创建菜单成功");
                }
            });
        };
        ...
        $scope.save = function () {
            ...
            $scope.fs.writeFile(config.save.url, JSON.stringify(button), function (error) {
                if (error) {
                    console.log(error);
                    alert('保存菜单失败');
                } else {
